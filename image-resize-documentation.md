...
- `src/imageSizeCalculator.ts` resolves dimensions using layered strategies so the plugin works across desktop, Android, and the web app. It owns fallback-dimension policy for both resource and external images; an invalid resource ID is still rejected outright, while a non-http(s) external source (relative path, `data:` URI) falls back to default dimensions so the resize dialog still opens. On desktop and the web app, resource bytes from the Data API are normalized into a `Uint8Array`, wrapped in a `Blob`, and measured through a temporary object URL without base64 conversion. Native mobile cannot transport the resource endpoint's Buffer through the plugin API, so it relies on editor-context measurement instead. External images are loaded without a referrer and without requiring CORS, since only their intrinsic dimensions are read.
- `src/dialogHandler.ts` builds and runs the resize dialog through an injected Joplin API boundary. It lazily creates one dialog handle and reuses it for every later invocation. The handle is never re-created: Joplin derives it from the dialog id and throws "View already added" when that id is registered twice, so a failed setup retries only the configuration steps (fit-to-content, scripts, buttons) against the handle that already exists. Overlapping opens are ignored, because Joplin resolves the *pending* first `open()` with null and hands the dialog to the later caller, which would silently cancel the invocation the user is looking at.
- `src/dialog/resizeDialog.ts` initializes each freshly rendered dialog root once, tracked by a `data-initialized` marker on the root. Joplin unmounts a dialog's webview when it closes and remounts it on the next open, and it applies the new HTML before loading the scripts, so the script runs once per open against markup that is already in place. Both apps also inject the script after the document is parsed, so no observer, `DOMContentLoaded` listener, or other re-entry mechanism is needed; the marker only keeps the exported function idempotent for callers that invoke it directly. If Joplin ever kept a dialog webview mounted across opens, this would need revisiting, because `setHtml` replaces the content element's markup without re-running the scripts.
...
- `src/viewerContextMenu.ts` receives those messages. For a context menu that did not originate in the editor, the filter takes a recent click message (waiting briefly if Joplin's menu request arrived first) and runs a read-only editor match command. A timed-out or editor-origin menu discards its click time, so a delayed message cannot target a later menu. Items are added only when the editor confirms the viewer image still matches the note source. The resize command revalidates and selects the image when chosen.
...
- On desktop and the web app, fall back to Blob object URL loading when resource bytes are available, always revoking the temporary URL after measurement.
